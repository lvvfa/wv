...
# 检查 root 权限
[[ $EUID -ne 0 ]] && LOGE "错误：您必须使用 root 权限运行此脚本！\n" && exit 1
...
uninstall() {
    confirm "您确定要卸载面板吗？xray 也会被卸载！" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
reset_user() {
    ...
    read -rp "是否要禁用当前配置的双重认证 (2FA)？ (y/n)： " twoFactorConfirm
    ...
    echo -e "${green} 请使用新的登录用户名和密码访问 X-UI 面板。请务必记住它们！ ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    read -rp "您确定要重置 Web 根路径吗？ (y/n)： " confirm
...
check_config() {
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        read -rp "立即为 IP 生成 SSL 证书？ [y/N]： " gen_ssl
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            stop >/dev/null 2>&1
            ssl_cert_issue_for_ip
            if [[ $? -eq 0 ]]; then
                echo -e "${green}访问 URL：https://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
                # ssl_cert_issue_for_ip 已经重启了面板，但确保它在运行
                start >/dev/null 2>&1
            else
                LOGE "IP 证书设置失败。"
                echo -e "${yellow}您可以尝试通过选项 18 (SSL 证书管理) 重试。${plain}"
                start >/dev/null 2>&1
            fi
        else
            echo -e "${yellow}访问 URL：http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}为了安全起见，请使用选项 18 (SSL 证书管理) 配置 SSL 证书${plain}"
        fi
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI "面板正在运行，无需再次启动。如果您需要重启，请选择重启"
    else
        if [[ $release == "alpine" ]]; then
...
firewall_menu() {
    echo -e "${green}\t1.${plain} ${green}安装${plain} 防火墙 (ufw)"
    echo -e "${green}\t2.${plain} 端口列表 [带编号]"
    echo -e "${green}\t3.${plain} ${green}开放${plain} 端口"
    echo -e "${green}\t4.${plain} ${red}删除${plain} 端口（从列表）"
...
open_ports() {
    # 提示用户输入要开放的端口
    read -rp "请输入您要开放的端口（例如 80,443,2053 或范围 400-500）： " ports
...
delete_ports() {
    ...
    if [[ $choice -eq 1 ]]; then
        # 按规则编号删除
        read -rp "请输入您要删除的规则编号（1, 2, 等）： " rule_numbers
        ...
        # 按端口删除
        read -rp "请输入您要删除的端口（例如 80,443,2053 或范围 400-500）： " ports
...
ssl_cert_issue_main() {
    ...
    echo -e "${green}\t6.${plain} 获取 IP 地址的 SSL 证书 (6天有效期，自动续期)"
    echo -e "${green}\t0.${plain} 返回主菜单"
    ...
    5)
        ...
        ;;
    6)
        echo -e "${yellow}IP 地址的 Let's Encrypt SSL 证书${plain}"
...
ssl_cert_issue_for_ip() {
    ...
    # 询问可选的 IPv6
    local ipv6_addr=""
    read -rp "您有要包含的 IPv6 地址吗？（留空跳过）： " ipv6_addr
    ...
    *)
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        LOGI "提醒：Let's Encrypt 仍然访问 80 端口；请将外部 80 端口转发到 ${WebPort} 进行验证。"
    fi
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            read -rp "请输入 acme.sh 独立监听器的另一个端口（留空中止）： " alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
                LOGE "端口 ${WebPort} 忙；无法继续签发。"
                return 1
            fi
    ...
    *)
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        ...
        1)
            ...
            ;;
        2)  
            LOGD "建议将 x-ui restart 放在最后，这样如果其他服务失败，它不会引发错误"
    ...
    # 证书安装成功后提示用户设置面板路径
    read -rp "您想将此证书设置为面板证书吗？(y/n)： " setPanel
...
iplimit_main() {
    ...
    5)
        ...
        ;;
    6)
        read -rp "请输入您要解封的 IP 地址： " unban_ip
        ip_validation
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        echo -e "${green}fail2ban.log 最近的系统封禁活动：${plain}"
        grep "3x-ipl" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}未发现最近的系统封禁活动${plain}"
        echo ""
    fi
    ...
    if [[ -f "${iplimit_banned_log_path}" ]]; then
        ...
        if [[ -s "${iplimit_banned_log_path}" ]]; then
            grep -v "INIT" "${iplimit_banned_log_path}" | tail -n 10 || echo -e "${yellow}未发现封禁条目${plain}"
        else
...
EOF
    ...
    echo -e "${green}IP 限制监狱文件已创建，封禁时间为 ${bantime} 分钟。${plain}"
}
...
│────────────────────────────────────────────────│
│  ${green}23.${plain} 开启 BBR                                  │
│  ${green}24.${plain} 更新 Geo 文件                             │
│  ${green}25.${plain} Speedtest 测速 (Ookla)                    │
╚────────────────────────────────────────────────╝
"
    show_status
